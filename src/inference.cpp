...
std::vector<Detection> Inference::runInference(const cv::Mat &input)
{
    ...
    Inference_time = (tt2 - tt1) / cv::getTickFrequency() * 1000;
...
*/
    ...
    NMS_time = (t2 - t1) / cv::getTickFrequency() * 1000.0;
...
cv::Mat Inference::showDetectionOnImage(const cv::Mat& image, std::vector<Detection> det) {
    int detections = det.size();
    ...
    // Inference ends here...
    std::string label = cv::format("Inference time: %.2f ms, NMS time: %.2f ms", Inference_time, NMS_time);
